...
const App = {
  ...
  init() {
    ...
    // Ctrl+C / Ctrl+X над планом: браузер шлёт copy/cut — кладём скопированное в системный буфер
    const onCopy = (e) => {
      const armed = App._copyArmed && performance.now() - App._copyArmed < 1000;
      App._copyArmed = 0;
      if (!armed || typingIn(e.target) || document.querySelector('dialog[open]') || !App.clipboard || !e.clipboardData) return;
  ...
  copy(opts = {}) {
    ...
    if (opts.keyboard) App._copyArmed = performance.now();   // событие copy/cut от Ctrl+C/X положит данные в системный буфер
